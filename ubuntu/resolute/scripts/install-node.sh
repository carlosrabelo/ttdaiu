#!/usr/bin/env bash
# install-node.sh for Ubuntu 26.04 Resolute
set -euo pipefail
...
_enable_universe_resolute() {
  ...
  log_info "Enabling universe repository (Resolute)..."
...
main() {
  ...
  # PATH for ~/.npm-global/bin lives in bash .profile (not .bashrc).
