...
# Antigravity, node, …). Canonical PATH entries live in bash .profile
# (ttdaiu profile_paths). Only removes `export PATH=…` / source forms and
# known installer comments — not the `if [ -d … ]; PATH=…` style used by
# Ubuntu/ttdaiu templates.
strip_ad_hoc_tool_path_rc() {
  local f
  # Match installer-style lines only (`export PATH=…`, source cargo/env, comments).
  # Do not match ttdaiu/Ubuntu `if [ -d … ]; PATH=…` blocks in .profile.
  local needle='\.cargo/env|Antigravity CLI|^[[:space:]]*#[[:space:]]*opencode[[:space:]]*$|^[[:space:]]*export PATH=.*(\.opencode/bin|\.npm-global/bin|\.local/bin)'
  for f in "${MAIN_HOME}/.profile" "${MAIN_HOME}/.bashrc" "${MAIN_HOME}/.bash_profile"; do
    ...
    sed -i -E \
      ...
      -e '/^[[:space:]]*export PATH=.*\.local\/bin/d' \
...
inject_block() {
  ...
  local start_marker="# >>> ttdaiu ${marker} start >>>"
  local end_marker="# <<< ttdaiu ${marker} end <<<"
  ...
  if grep -qF "${start_marker}" "${target_file}"; then
    ...
    # Ensure there is a newline at the end of the block
    [[ "$(tail -c1 "${temp_file}")" != $'\n' ]] && echo "" >> "${temp_file}"
...
inject_block_if_missing() {
  ...
  # If the file already has the start marker, we want to update it
  if [[ -f "${target_file}" ]] && grep -qF "${start_marker}" "${target_file}"; then
    inject_block "${marker}" "${target_file}" "${source_file}"
    return 0
  fi

  # Otherwise, check if the search string already exists in the file
  if [[ -f "${target_file}" ]] && grep -qF "${search_str}" "${target_file}"; then
    log_info "Block '${marker}' already satisfied in ${target_file} (found '${search_str}') — skipping"
    return 0
  fi

  # If not present at all, inject it
  inject_block "${marker}" "${target_file}" "${source_file}"
}
